...
package net.mcreator.frostpack.world.biome;
...
import net.mcreator.frostpack.block.SkyDemensionMainblock2Block;
import net.mcreator.frostpack.block.CloudBlock;
...
public class SkyBiomeBiome extends FrostPackElements.ModElement {
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.NONE)
					...
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(CloudBlock.block.getDefaultState(),
							SkyDemensionMainblock2Block.block.getDefaultState(), SkyDemensionMainblock2Block.block.getDefaultState())));
	...
	static class CustomTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
		...
		public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldgen, Random rand, BlockPos position, MutableBoundingBox bbox) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == CloudBlock.block.getDefaultState().getBlock()
							|| ground == SkyDemensionMainblock2Block.block.getDefaultState().getBlock())
							&& (ground2 == CloudBlock.block.getDefaultState().getBlock()
									|| ground2 == SkyDemensionMainblock2Block.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), SkyDemensionMainblock2Block.block.getDefaultState(), bbox);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == SkyDemensionMainblock2Block.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, SkyDemensionMainblock2Block.block.getDefaultState(),
													bbox);
										}
									}
								}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, CloudBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == CloudBlock.block.getDefaultState().getBlock()
					|| blockType == SkyDemensionMainblock2Block.block.getDefaultState().getBlock()
					|| blockType == CloudBlock.block.getDefaultState().getBlock()
					|| blockType == SkyDemensionMainblock2Block.block.getDefaultState().getBlock();
		}
